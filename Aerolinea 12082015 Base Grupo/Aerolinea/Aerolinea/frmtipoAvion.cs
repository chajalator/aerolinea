    ...
    public partial class frmtipoAvion : Form
    {
        ...
        private void funbuscarA() {
            try {
                ...
                claseUsuario.funobtenerBitacora(claseUsuario.varibaleUsuario, "Busco Registro", "TrTipoAvion");
        ...
        private void funbuscarV() {
            try {
                ...
                grdVuelo.AutoResizeColumns();
                //INGRESO BITACORA PROGRAMADOR Y ANALISTA GERADO SANTIZO
                claseUsuario.funobtenerBitacora(claseUsuario.varibaleUsuario, "Busco  Registro", "MaTipoVuelo");
                //FIN iNGRESO bITACORA
                clasconexion.funobtenerConexion().Close();
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
                ...
                if (txtavion.Text != "")
                {
                    try
                    {
                        ...
                        using (clasconexion.funobtenerConexion())
                        {
                            ...
                            //INGRESO BITACORA PROGRAMADOR GERANDO SANTIZO
                            claseUsuario.funobtenerBitacora(claseUsuario.varibaleUsuario, "Ingreso Tipo de Avion", "TrTipoAvion");
                            //FIN iNGRESO bITACORA
                            clasconexion.funobtenerConexion().Close();
                            funlimpiar();
                            funconsultaAvion();
                ...
                if (txtvuelo.Text != "")
                {
                    try
                    {
                        ...
                        using (clasconexion.funobtenerConexion())
                        {
                            ...
                            //INGRESO BITACORA PROGRAMADOR GERANDO SANTIZO
                            claseUsuario.funobtenerBitacora(claseUsuario.varibaleUsuario, "Ingreso Tipo de Vuelo", "MaTipoVuelo");
                            //FIN iNGRESO bITACORA
                            clasconexion.funobtenerConexion().Close();
                            funlimpiar();
                            funconsultaVuelo();
